...
torch::Tensor get_tile_bin_edges_tensor(
    ...
    CHECK_INPUT(isect_ids_sorted);
    int tiles_x = std::get<0>(tile_bounds);
    int tiles_y = std::get<1>(tile_bounds);
...
> project_gaussians_2d_scale_rot_forward_tensor(
    ...
    // printf("hello! this is in project_gaussians_2d_scale_rot_forward_tensor\n");
    // printf("num_points: %d\n", num_points);
    // printf("img_width: %d\n", img_width);
    // printf("img_height: %d\n", img_height);
    // printf("tile_bounds: %d %d %d\n", std::get<0>(tile_bounds), std::get<1>(tile_bounds), std::get<2>(tile_bounds));
    // printf("clip_thresh: %f\n", clip_thresh);

    project_gaussians_2d_scale_rot_forward_kernel<<<(num_points + N_THREADS - 1) / N_THREADS, N_THREADS>>>(
        num_points,
...
> project_gaussians_2d_scale_rot_backward_tensor(
    ...
    CUDA_CALL(cudaDeviceSynchronize());
    // printf("project_gaussians_2d_scale_rot_backward_tensor done\n");
