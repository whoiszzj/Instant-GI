...
inline __device__ void cov2d_to_conic_vjp(
    ...
    glm::mat2 X = glm::mat2(conic.x, conic.y, conic.y, conic.z); // 保持列主序矩阵
    glm::mat2 G = glm::mat2(v_conic.x, v_conic.y, v_conic.y, v_conic.z);
    glm::mat2 v_Sigma = -X * G * X; // 保持列主序乘法

    // 调整访问方式，因为 v_Sigma 是列主序存储的
    v_cov2d.x = v_Sigma[0][0];  // Sigma_11
    v_cov2d.y = v_Sigma[0][1];  // Sigma_12
    v_cov2d.z = v_Sigma[1][1];  // Sigma_22
}
...
inline __device__ glm::mat2 rotmat2d(const float rot) {
    ...
    // 保持列主序
    glm::mat2 R = glm::mat2(cosr, sinr, -sinr, cosr);
    return R;
}

inline __device__ glm::mat2 rotmat2d_gradient(const float rot) {
    // quat to rotation matrix gradient
    float cosr = cos(rot);
    float sinr = sin(rot);

    // 保持列主序
    glm::mat2 R = glm::mat2(-sinr, cosr, -cosr, -sinr);
    return R;
}
